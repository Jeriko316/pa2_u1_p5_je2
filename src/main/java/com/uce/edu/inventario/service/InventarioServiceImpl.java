...
public class InventarioServiceImpl implements IInventarioService {
	...
	@Autowired
	private IProductoRepository prodcutoRepository;
	...
	public void registrar(String codigoBodega, String codigoBarras, Integer stock) {
		...
		Producto producto = this.prodcutoRepository.seleccionar(codigoBarras);
		Integer stockActual = producto.getStock();
		Integer stockFinal = stockActual + stock;
		producto.setStock(stockFinal);
		this.prodcutoRepository.actualizar(producto);

		Inventario inv = new Inventario();
		inv.setCodigo("234234234");
		inv.setFechaIngreso(LocalDateTime.now());
		inv.setBodega(bodega);
		inv.setProducto(producto);
		
		
		this.iInventarioRepository.insertar(inv); //crear el inventario
